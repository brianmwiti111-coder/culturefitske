  ...
  try {
    ...
    if (variantsRaw) {
      try {
        ...
        const valid = parsed.filter((v) => SIZES.includes(v.size) && SLEEVES.includes(v.sleeve));
        if (valid.length === 0) {
          return NextResponse.json({ error: "Select at least one size to offer." }, { status: 400 });
        }
        variants = valid.map((v) => ({ size: v.size, sleeve: v.sleeve, stock: Math.max(0, Number(v.stock) || 0) }));
      } catch {

...
// for every photo angle, but close enough to give a real preview.
function PhotoCustomizationPreview({ photoUrl, name, number, font, namePosition = "below", designImage }) {
  const f = FONTS.find((x) => x.id === font) || FONTS[0];
  const numberTop = namePosition === "above" ? "54%" : "42%";
  const nameTop = namePosition === "above" ? "38%" : "62%";

  return (
    <div style={{ position: "relative", width: "100%", borderRadius: 8, overflow: "hidden" }}>
      <img src={photoUrl} alt="Customization preview" style={{ width: "100%", display: "block" }} />
      {designImage && (
        <div style={{
          position: "absolute", top: "30%", left: "50%", transform: "translate(-50%, -50%)",
          width: "18%", aspectRatio: "1", borderRadius: "50%", overflow: "hidden",
          border: "2px solid rgba(255,255,255,0.85)", boxShadow: "0 2px 8px rgba(0,0,0,0.4)",
        }}>
          <img src={designImage} alt="Custom design" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
      )}
      {number && (
        <div style={{
          position: "absolute", top: numberTop, left: "50%", transform: "translate(-50%, -50%)",
          fontFamily: f.family, fontWeight: f.weight, fontStyle: f.style || "normal",
          fontSize: "clamp(28px, 9vw, 64px)", color: "#ffffff",
          textShadow: "0 2px 6px rgba(0,0,0,0.6), 0 0 1px rgba(0,0,0,0.8)",
          lineHeight: 1, whiteSpace: "nowrap",
        }}>{number}</div>
      )}
      {name && (
        <div style={{
          position: "absolute", top: nameTop, left: "50%", transform: "translate(-50%, -50%)",
          fontFamily: f.family, fontWeight: f.weight, fontStyle: f.style || "normal",
          fontSize: "clamp(11px, 3vw, 18px)", letterSpacing: "0.08em", color: "#ffffff",
            ...
            <>
              {product.customizationPhoto ? (
                <PhotoCustomizationPreview
                  ...
                  name={name} number={number} font={font} namePosition={namePosition} designImage={designImage}
                />
              ) : (
...
// PRODUCT REVIEWS
// ---------------------------------------------------------------
function ProductReviews({ productId, token, user }) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await apiFetch("/api/reviews", { method: "POST", token, body: { productId, rating, comment } });
      setComment("");
      setRating(5);
      load();
    } catch (err) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-5 pb-16">
      <SectionLabel eyebrow="From other customers" title="Reviews" />

      {loading ? (
        <p style={{ fontFamily: FONT_BODY, color: C.mute }}>Loading reviews...</p>
      ) : reviews.length === 0 ? (
        <p style={{ fontFamily: FONT_BODY, color: C.mute, marginBottom: 24 }}>No reviews yet — be the first.</p>
      ) : (
        <div className="flex flex-col gap-3 mb-8">
          {reviews.map((r) => (
            <div key={r._id || r.id} style={{ background: C.bgCard, border: `1px solid ${C.line}`, borderRadius: 10, padding: 16 }}>
              <div className="flex items-center gap-2 mb-2">
                <div className="flex" style={{ gap: 2 }}>
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={14} fill={i < r.rating ? C.gold : "none"} color={C.gold} />
                  ))}
                </div>
                <span style={{ fontFamily: FONT_BODY, fontSize: 13, fontWeight: 600, color: C.white }}>{r.userName}</span>
              </div>
              {r.comment && <p style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.mute, margin: 0 }}>{r.comment}</p>}
            </div>
          ))}
        </div>
      )}
      ...
      {user ? (
        <div style={{ background: C.bgCard, border: `1px solid ${C.line}`, borderRadius: 10, padding: 20, maxWidth: 480 }}>
          <div style={{ fontFamily: FONT_BODY, fontSize: 13, fontWeight: 700, color: C.white, marginBottom: 10 }}>Leave a review</div>
          <div className="flex gap-1 mb-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <button key={i} onClick={() => setRating(i + 1)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2 }}>
                <Star size={22} fill={i < rating ? C.gold : "none"} color={C.gold} />
              </button>
            ))}
          </div>
          <textarea
            ...
            placeholder="What did you think of the fit and quality? (optional)"
            rows={3}
            style={{ width: "100%", background: C.bg, border: `1px solid ${C.line}`, borderRadius: 6, padding: "9px 11px", color: C.white, fontFamily: FONT_BODY, marginBottom: 12, resize: "vertical" }}
          />
          {error && <div style={{ fontFamily: FONT_BODY, fontSize: 12, color: "#ff9088", marginBottom: 10 }}>{error}</div>}
...
function MyOrders({ token, user, setView }) {
  ...
  if (!user) {
    return (
      <div className="max-w-md mx-auto px-5 py-24 text-center">
        <p style={{ fontFamily: FONT_BODY, color: C.mute, marginBottom: 16 }}>Log in to see your order history.</p>
        <Button onClick={() => setView("login")}>Log In</Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 py-14">
      <SectionLabel eyebrow="Your account" title="My Orders" />
      {loading ? (
        <p style={{ fontFamily: FONT_BODY, color: C.mute }}>Loading your orders...</p>
      ) : error ? (
        <p style={{ fontFamily: FONT_BODY, color: "#ff9088" }}>{error}</p>
      ) : orders.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 0" }}>
          <p style={{ fontFamily: FONT_BODY, color: C.mute, marginBottom: 16 }}>You haven't placed any orders yet.</p>
          <Button onClick={() => setView("shop")}>Browse Kits</Button>
        </div>
      ) : (
            ...
            return (
              <div key={o.id} style={{ background: C.bgCard, border: `1px solid ${C.line}`, borderRadius: 10, padding: 18 }}>
                <div className="flex flex-wrap justify-between items-start gap-3 mb-3">
                  <div style={{ fontFamily: FONT_DISPLAY, fontSize: 16, color: C.white }}>{o.orderNumber}</div>
                  <Badge tone="gold">{o.status}</Badge>
                </div>
                <div style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.mute, marginBottom: 10 }}>
                  {o.items.map((i) => `${i.quantity}× ${i.productName} (${i.size}/${i.sleeve})`).join(", ")}
                </div>
                <div className="flex justify-between items-center">
                  <span style={{ fontFamily: FONT_BODY, fontSize: 12, color: C.mute }}>{o.deliveryLocation}</span>
                  <span style={{ fontFamily: FONT_DISPLAY, fontSize: 16, color: C.gold }}>
                    {o.deliveryFee != null ? `KSh ${total.toLocaleString()}` : `KSh ${o.subtotal.toLocaleString()} + delivery TBC`}
                  </span>
                </div>
              </div>
            );
...
function AddProductForm({ addProduct, products }) {
  ...
  const removePhoto = (idx) => {
    ...
    setCustomizationPhotoIndex((cur) => {
      if (cur === null) return null;
      if (cur === idx) return null; // the chosen photo was removed
      return cur > idx ? cur - 1 : cur; // shift down to stay pointed at the same photo
    });
  };
    ...
    try {
      ...
      for (const size of selectedSizes) {
        for (const sleeve of selectedSleeves) {
          variants.push({ size, sleeve, stock: Number(initialStock[`${size}-${sleeve}`]) || 0 });
        }
      }
      fd.append("variants", JSON.stringify(variants));
      if (customizationPhotoIndex !== null) fd.append("customizationPhotoIndex", String(customizationPhotoIndex));
    ...
    <div style={{ background: C.bgCard, border: `1px solid ${C.gold}`, borderRadius: 10, padding: 20, marginBottom: 24 }}>
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2 flex-wrap mb-3">
          {SIZES.map((size) => (
            <button key={size} onClick={() => toggleSize(size)} style={{
              width: 44, height: 36, borderRadius: 6, fontFamily: FONT_BODY, fontWeight: 700, fontSize: 12,
              border: `1px solid ${selectedSizes.includes(size) ? C.gold : C.line}`,
              background: selectedSizes.includes(size) ? "rgba(217,169,78,0.12)" : "transparent",
              color: selectedSizes.includes(size) ? C.gold : C.mute, cursor: "pointer",
            }}>{size}</button>
          ))}
        </div>
        ...
        {selectedSizes.length === 0 || selectedSleeves.length === 0 ? (
          ...
          <>
            <div style={{ fontFamily: FONT_BODY, fontSize: 12, color: C.mute, margin: "12px 0 6px" }}>Starting stock per variant</div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {selectedSizes.map((size) => selectedSleeves.map((sleeve) => {
                ...
                return (
                  <div key={key} style={{ background: C.bgRaise, border: `1px solid ${C.line}`, borderRadius: 6, padding: 8 }}>
                    ...
                    <input
                      type="number"
                      value={initialStock[key] ?? ""}
                      onChange={(e) => setInitialStock((s) => ({ ...s, [key]: e.target.value }))}
                      placeholder="0"
                      style={{ width: 56, background: C.bg, border: `1px solid ${C.line}`, borderRadius: 4, padding: "4px 6px", color: C.white, fontFamily: FONT_BODY, fontSize: 12 }}
                    />
                  </div>
                );
              }))}
          ...
          {form.photos.map((src, i) => {
            ...
            return (
              <div key={i} style={{ position: "relative", width: 72, height: 72 }}>
                <img src={src} alt={`Kit photo ${i + 1}`} style={{
                  width: "100%", height: "100%", objectFit: "cover", borderRadius: 6,
                  border: `2px solid ${isChosen ? C.gold : C.line}`,
                }} />
                <button
                ...
                <button
                  onClick={() => setCustomizationPhotoIndex(isChosen ? null : i)}
                  title={isChosen ? "Used for customization preview" : "Use this photo for customization"}
                  style={{
                    position: "absolute", bottom: -6, left: -6, background: isChosen ? C.gold : "rgba(5,11,26,0.85)",
                    border: "none", borderRadius: 999, width: 20, height: 20, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer",
                  }}
                >
                  <Sparkles size={11} color={isChosen ? "#1a1204" : C.mute} />
                </button>
              </div>
            );
          })}
        ...
        {form.photos.length > 0 && (
          <div style={{ fontFamily: FONT_BODY, fontSize: 11, color: C.mute, marginBottom: 4 }}>
            {customizationPhotoIndex !== null
              ? "Tap the sparkle again to unpick — customization will preview on the highlighted photo."
...
function StockEditor({ product, updateStock }) {
  ...
  const [addingSleeve, setAddingSleeve] = useState("");
  const [addingStock, setAddingStock] = useState("0");

  const availableToAdd = [];
  for (const size of SIZES) {
    for (const sleeve of SLEEVES) {
      if (!existingKeys.includes(`${size}-${sleeve}`)) availableToAdd.push({ size, sleeve });
    }
  }

  const hasChanges = existingKeys.some((key) => drafts[key] != null && Number(drafts[key]) !== product.stock[key]);

  const saveAll = async () => {
    setError("");
    setSaving(true);
    try {
      const changed = existingKeys.filter((key) => drafts[key] != null && Number(drafts[key]) !== product.stock[key]);
      for (const key of changed) {
        const [size, sleeve] = key.split("-");
        await updateStock(product.id, size, sleeve, Number(drafts[key]));
      }
      setDrafts({});
      setSaved(true);
      setTimeout(() => setSaved(false), 1800);
    } catch (err) {
      setError(err.message || "Could not save stock changes.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const addVariant = async () => {
    if (!addingSize || !addingSleeve) return;
    setError("");
    try {
      await updateStock(product.id, addingSize, addingSleeve, Number(addingStock) || 0);
      setAddingSize("");
      setAddingSleeve("");
      setAddingStock("0");
    } catch (err) {
  ...
  return (
    <div className="mt-3">
            ...
            <div key={key} style={{ background: C.bgRaise, border: `1px solid ${C.line}`, borderRadius: 6, padding: 8 }}>
              ...
              <input
                ...
                style={{
                  ...
                  borderRadius: 4, padding: "5px 7px", color: C.white, fontFamily: FONT_BODY, fontSize: 13,
                }}
              />
            </div>
          );
      ...
      <Button onClick={saveAll} disabled={!hasChanges || saving} style={{ padding: "9px 18px", fontSize: 13 }}>
        {saving ? "Saving..." : saved ? <><Check size={14} /> Stock Updated</> : "Save Changes"}
      </Button>

      {availableToAdd.length > 0 && (
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${C.line}` }}>
          <div style={{ fontFamily: FONT_BODY, fontSize: 12, color: C.mute, marginBottom: 8 }}>Add a size not yet offered</div>
          <div className="flex items-end gap-2 flex-wrap">
            <select
              value={addingSize ? `${addingSize}-${addingSleeve}` : ""}
              onChange={(e) => {
                const [size, sleeve] = e.target.value.split("-");
                setAddingSize(size || "");
                setAddingSleeve(sleeve || "");
              }}
              style={{ background: C.bg, border: `1px solid ${C.line}`, borderRadius: 6, padding: "8px 10px", color: C.white, fontFamily: FONT_BODY, fontSize: 13 }}
            >
              <option value="">Choose size / sleeve</option>
              {availableToAdd.map(({ size, sleeve }) => (
                <option key={`${size}-${sleeve}`} value={`${size}-${sleeve}`}>{size} · {sleeve}</option>
              ))}
            </select>
            <input
              type="number"
              value={addingStock}
              onChange={(e) => setAddingStock(e.target.value)}
              placeholder="Starting stock"
              style={{ width: 100, background: C.bg, border: `1px solid ${C.line}`, borderRadius: 6, padding: "8px 10px", color: C.white, fontFamily: FONT_BODY, fontSize: 13 }}
            />
            <Button variant="outline" onClick={addVariant} disabled={!addingSize} style={{ padding: "8px 14px", fontSize: 13 }}>
              <Plus size={14} /> Add
            </Button>
          </div>
        </div>
...
function CustomizationPhotoPicker({ product, setCustomizationPhoto }) {
  ...
  const choose = async (url) => {
    ...
    setSaving(true);
    try {
      await setCustomizationPhoto(product.id, url === product.customizationPhoto ? null : url);
    } catch (err) {
      setError(err.message || "Could not update this.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!product.photos || product.photos.length === 0) {
    return <p style={{ fontFamily: FONT_BODY, fontSize: 12, color: C.mute, marginTop: 10 }}>No photos uploaded for this kit yet.</p>;
  }

  return (
    <div className="mt-3">
      <p style={{ fontFamily: FONT_BODY, fontSize: 12, color: C.mute, marginBottom: 8 }}>
        Pick which photo customers see their name/number/design overlaid on. Leave none selected to use the drawn preview instead.
      </p>
      <div className="flex gap-2 flex-wrap">
        {product.photos.map((url, i) => {
          const isChosen = product.customizationPhoto === url;
          return (
            <button
              key={i}
              onClick={() => choose(url)}
              disabled={saving}
              style={{ padding: 0, border: `2px solid ${isChosen ? C.gold : C.line}`, borderRadius: 6, cursor: "pointer", background: "none", position: "relative" }}
            >
              <img src={url} alt={`Photo ${i + 1}`} style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 4, display: "block" }} />
              {isChosen && (
                <div style={{ position: "absolute", top: -6, right: -6, background: C.gold, borderRadius: 999, width: 18, height: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Sparkles size={11} color="#1a1204" />
                </div>
              )}
            </button>
          );
      ...
      {error && <div style={{ fontFamily: FONT_BODY, fontSize: 12, color: "#ff9088", marginTop: 8 }}>{error}</div>}
    </div>
  );
}
          ...
          return (
            <div key={p.id} style={{ background: C.bgCard, border: `1px solid ${C.line}`, borderRadius: 10, padding: 18 }}>
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={() => setPhotoExpandedId(photoExpanded ? null : p.id)}
                  style={{ background: "none", border: "none", color: C.gold, fontFamily: FONT_BODY, fontSize: 12, marginTop: 10, cursor: "pointer", padding: 0 }}
                >
                  {photoExpanded ? "Hide customization photo" : "Set customization photo"}
                </button>
              </div>
